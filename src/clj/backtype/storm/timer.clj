...
(defnk schedule-millis [timer delay-millis afn :check-active true]
  (when check-active (check-active! timer))
  (let [id (uuid)
        ^PriorityQueue queue (:queue timer)]
    (locking (:lock timer)
      (.add queue [(+ (current-time-millis) delay-millis) afn id])
      )))

(defnk schedule [timer delay-secs afn :check-active true]
  (schedule-millis timer (* 1000 delay-secs) afn :check-active true))

(defn schedule-recurring-millis [timer delay-millis recur-millis afn]
  (schedule-millis timer
    delay-millis
    (fn this []
      (afn)
      (schedule-millis timer recur-millis this :check-active false)) ; this avoids a race condition with cancel-timer
    ))

(defn schedule-recurring [timer delay-secs recur-secs afn]
  (schedule-recurring-millis timer (* 1000 delay-secs) (* 1000 recur-secs) afn))
