...
(defnk do-executor-heartbeats [worker :executors nil]
  ...
  (let [stats (if-not executors
        ...
        zk-hb {:storm-id (:storm-id worker)
               ...
               :topology-version (:topology-version worker)
               }]
...
(defn worker-data [conf mq-context storm-id assignment-id port worker-id topology-version]
  (let [cluster-state (cluster/mk-distributed-cluster-state conf)
        ...
        topology (read-supervisor-topology conf topology-version)]
    (recursive-map
      :conf conf
...
(defserverfn mk-worker [conf shared-mq-context storm-id assignment-id port worker-id topology-version]
  ...
  (let [worker (worker-data conf shared-mq-context storm-id assignment-id port worker-id topology-version)
...
(defn -main [storm-id assignment-id port-str worker-id topology-version]  
  (let [conf (read-storm-config)]
    ...
    (mk-worker conf nil (java.net.URLDecoder/decode storm-id) assignment-id (Integer/parseInt port-str) worker-id topology-version)))
